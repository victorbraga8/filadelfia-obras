    ...
    const handleScroll = () => {
      ...
      if (currentSection) {
        setActiveSection((previousValue) =>
          previousValue === currentSection.id
            ? previousValue
            : currentSection.id,
        );
      }
            ...
            {NAV_ITEMS.map((item) => (
              <li key={item.id}>
                <Button
                  asChild
                  variant="ghost"
                  className={`group relative h-auto !border-0 px-2 py-2 text-sm font-medium tracking-widest uppercase !text-white !shadow-none no-underline transition-colors hover:!text-white focus:!outline-none focus-visible:!border-transparent focus-visible:!outline-none focus-visible:!ring-0 ${
                    isScrolled
                      ? "!bg-transparent hover:!bg-transparent active:!bg-transparent"
                      : "!bg-slate-950/60 hover:!bg-slate-950/75 active:!bg-slate-950/75"
                  }`}
                >
                  <a
                    href={`#${item.id}`}
              ...
              {NAV_ITEMS.map((item) => (
                <li key={item.id} className="w-full text-center">
                  <Button
                    asChild
                    variant="ghost"
                    className={`w-full !border-0 !bg-transparent text-2xl font-bold tracking-widest uppercase !shadow-none no-underline transition-colors hover:!bg-transparent focus-visible:!border-transparent focus-visible:!bg-transparent focus-visible:!ring-0 active:!bg-transparent ${
                      activeSection === item.id
                        ? "text-blue-500"
